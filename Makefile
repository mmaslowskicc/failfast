...
shell: build
	$(DOCKER) run -v $$PWD:/code -v failfast-tox:/code/.tox --rm -it $(SERVICE_NAME):latest /bin/bash

test: build
	# Required to clean ephemeral storage after running tests (even if they fail)
	$(DOCKER) run -v $$PWD:/code -v failfast-tox:/code/.tox --rm $(SERVICE_NAME):latest tox
